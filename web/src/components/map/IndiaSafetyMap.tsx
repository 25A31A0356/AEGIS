...
const createSOSIcon = (status: string = 'PENDING', label?: string) => {
  const isIncoming = status === 'PENDING' || status === 'incoming';
  const isEnRoute = status === 'RESPONDER_EN_ROUTE' || status === 'dispatching' || status === 'ACCEPTED';
  const isOnSite = status === 'ON_SITE' || status === 'on_scene';
  ...
  return L.divIcon({
    ...
    html: `
      <div class="relative flex items-center justify-center cursor-pointer group">
        <div class="absolute w-9 h-9 rounded-full ${isIncoming ? 'animate-ping' : ''} opacity-40" style="background-color: ${color};"></div>
        <div class="w-8 h-8 rounded-full border-2 border-white shadow-elevated flex items-center justify-center text-white text-[11px] font-bold font-mono transition-transform group-hover:scale-110" style="background-color: ${color};">
          SOS
        </div>
        ${label ? `<span class="absolute -bottom-4 px-1.5 py-0.2 rounded bg-slate-900 text-white text-[8px] font-mono whitespace-nowrap shadow-xs">${label}</span>` : ''}
      </div>
    `,
    iconSize: [36, 36],
    iconAnchor: [18, 18],
...
const createResponderUnitIcon = (unitType?: string, unitName?: string) => {
  const icon = unitType?.includes('Boat') ? '🚤' : unitType?.includes('Ambulance') ? '🚑' : '🚒';
  return L.divIcon({
    className: 'custom-responder-marker',
    html: `
      <div class="relative flex flex-col items-center justify-center -translate-y-2 cursor-pointer group">
        <div class="absolute -top-1 w-9 h-9 rounded-full bg-sky-500/40 animate-ping"></div>
        <div class="w-8 h-8 rounded-full bg-slate-900 border-2 border-sky-400 shadow-elevated flex items-center justify-center text-sm transition-transform group-hover:scale-110">
          ${icon}
        </div>
        <div class="w-1.5 h-1.5 rotate-45 -mt-0.5 bg-slate-900 shadow-xs"></div>
        <span class="mt-0.5 px-1.5 py-0.2 rounded bg-sky-950 text-sky-300 text-[8px] font-mono font-bold whitespace-nowrap shadow-xs">
          ${unitName ? unitName.substring(0, 16) : 'RESPONDER'}
        </span>
      </div>
    `,
    iconSize: [34, 40],
  ...
  return (
    <div className={`relative w-full ${heightClass} rounded-2xl overflow-hidden border border-slate-200 shadow-card bg-slate-100`}>
      ...
      >
        ...
        {/* 5. SOS Beacons (Authoritative Realtime Stream & Responder Units) */}
        {layers.sosBeacons &&
          sosBeacons.map((sos) => (
            <React.Fragment key={sos.id}>
              {/* Requester SOS Marker */}
              <Marker
                position={sos.coordinates}
                icon={createSOSIcon(sos.triageStatus, sos.anonymousAlias || sos.id)}
                eventHandlers={{
                  ...
                  <div className="p-3 max-w-xs font-sans">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className={`text-[10px] font-mono font-bold uppercase px-1.5 py-0.5 rounded ${
                        sos.triageStatus === 'RESOLVED' ? 'bg-emerald-100 text-emerald-800' :
                        sos.triageStatus === 'ON_SITE' ? 'bg-purple-100 text-purple-800' :
                        sos.triageStatus === 'RESPONDER_EN_ROUTE' ? 'bg-sky-100 text-sky-800' :
                    ...
                    <div className="text-[10px] text-slate-500 font-mono bg-slate-50 p-2 rounded border border-slate-200 mb-2 space-y-0.5">
                      <div>Contact: <span className="font-bold text-slate-700">{sos.phoneMasked || sos.anonymousAlias || 'Redacted'}</span></div>
                      <div>Persons: <strong>{sos.personsCount}</strong> • Battery: {sos.batteryPercent}% • GPS ±{sos.gpsAccuracyMeters}m</div>
                      {sos.assignedUnit && (
                        <div className="text-sky-700 font-bold border-t border-slate-200 pt-1 mt-1">
                          Unit: {sos.assignedUnit.callsign} (ETA: {sos.assignedUnit.etaMinutes}m)
                        </div>
                      )}
                    </div>
                    <button
              ...
              {/* Responder Unit Marker if assigned & active */}
              {sos.assignedUnit?.responderCoordinates && sos.triageStatus !== 'RESOLVED' && sos.triageStatus !== 'CANCELLED' && (
                <Marker
                  position={sos.assignedUnit.responderCoordinates}
                  icon={createResponderUnitIcon(sos.assignedUnit.unitType, sos.assignedUnit.callsign)}
                  eventHandlers={{
                    click: () => onSelectSOS && onSelectSOS(sos.id),
                  }}
                >
                  <Popup>
                    <div className="p-2.5 max-w-xs font-sans text-xs">
                      <div className="font-bold text-sky-800 flex items-center justify-between mb-1">
                        <span>{sos.assignedUnit.callsign}</span>
                        <span className="text-[9px] font-mono uppercase bg-sky-100 text-sky-800 px-1.5 py-0.5 rounded">
                          {sos.assignedUnit.unitType}
                        </span>
                      </div>
                      <div className="text-slate-600 mb-1">
                        Commander: <strong>{sos.assignedUnit.commanderName}</strong> ({sos.assignedUnit.commanderContact})
                      </div>
                      <div className="text-[10px] font-mono text-slate-500 bg-sky-50 p-1.5 rounded border border-sky-200">
                        Target SOS: <strong className="text-red-700">{sos.id}</strong> • ETA: {sos.assignedUnit.etaMinutes}m • Dist: {sos.assignedUnit.distanceKm} km
                      </div>
                    </div>
                  </Popup>
                </Marker>
              ...
              {/* Dynamic Connecting Polyline between Responder and Requester */}
              {sos.assignedUnit?.responderCoordinates && sos.triageStatus !== 'RESOLVED' && sos.triageStatus !== 'CANCELLED' && (
                <Polyline
                  positions={[sos.assignedUnit.responderCoordinates, sos.coordinates]}
                  pathOptions={{
                    color: sos.triageStatus === 'ON_SITE' ? '#8B5CF6' : '#0284C7',
                    weight: 3,
                    dashArray: '6, 8',
                    opacity: 0.85,
                  }}
