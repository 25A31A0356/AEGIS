...
export const navItemsConfig: NavItem[] = [
  { id: 'homepage', labelKey: 'nav.homepage', defaultLabel: 'Homepage', icon: Home },
  { id: 'analytics', labelKey: 'nav.analytics', defaultLabel: 'Analytics', icon: BarChart3, badge: 'LIVE' },
  { id: 'safety', labelKey: 'nav.safety', defaultLabel: 'Safety', icon: ShieldCheck },
  { id: 'reports', labelKey: 'nav.reports', defaultLabel: 'Reports', icon: FileText },
  { id: 'live-map', labelKey: 'nav.liveMap', defaultLabel: 'Live Map', icon: Radio, badge: 'GIS' },
];
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside
      className={`relative flex flex-col justify-between h-full bg-[#075B8A] dark:bg-[#071828] text-white select-none transition-all duration-300 ease-in-out z-30 shadow-xl border-r border-white/5 dark:border-[#1E3347] ${
        isCollapsed ? 'w-18' : 'w-[216px]'
      }`}
    >
      {/* Top Header & Logo */}
      <div>
        <div className="flex items-center justify-between px-4 py-5 border-b border-white/10 dark:border-white/5">
          <div
            ...
            {!isCollapsed && (
              <div className="overflow-hidden">
                ...
                <p className="text-[9px] tracking-wider text-[#A7D7E8] font-semibold uppercase leading-tight mt-0.5 truncate">
                  {t('app.tagline', 'MULTI-HAZARD EARLY WARNING')}
                </p>
              </div>
            )}
          </div>
