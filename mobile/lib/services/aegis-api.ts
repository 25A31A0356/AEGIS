...
class AegisApiServiceClass {
  ...
  /**
   * Register push notification token with Aegis backend
   */
  async registerDeviceToken(token: string): Promise<boolean> {
    try {
      const endpoint = "/api/trpc/aegis.registerDeviceToken";
      await apiCall(endpoint, {
        method: "POST",
  ...
  async syncOfflineQueue(): Promise<{ syncedCount: number; remainingCount: number }> {
    ...
    for (const item of queue) {
      try {
        ...
        } else if (item.type === "sos_beacon") {
          await apiCall("/api/trpc/sosBeacon.create", {
            method: "POST",
            body: JSON.stringify(item.payload),
          });
        ...
        } else if (item.type === "sos_incident") {
